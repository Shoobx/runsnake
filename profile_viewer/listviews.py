...
class DataView(wx.ListCtrl):
    ...
    def SetColumns(self, columns, sortOrder=None):
        ...
        self.columns = columns
        self.sortOrder = [(x.defaultOrder, x) for x in self.columns
                          if x.sortDefault]
        self.CreateColumns()
    ...
    def OnNodeActivated(self, event):
        ...
        try:
            ...
            log.warn(
                _('Invalid index in node activated: %(index)s'),
                index=event.GetIndex())
        else:
            wx.PostEvent(self,
                         squaremap.SquareActivationEvent(
                             node=node, point=None, map=None))
    ...
    def OnNodeSelected(self, event):
        ...
        try:
            ...
            log.warn(
                _('Invalid index in node selected: %(index)s'),
                index=event.GetIndex())
        else:
            if node is not self.selected_node:
                wx.PostEvent(self,
                             squaremap.SquareSelectionEvent(
                                 node=node, point=None, map=None))
    ...
    def OnMouseMove(self, event):
        ...
        if item > -1:
            try:
                ...
                log.warn(
                    _('Invalid index in mouse move: %(index)s'),
                    index=event.GetIndex())
            else:
                wx.PostEvent(self,
                             squaremap.SquareHighlightEvent(
                                 node=node, point=point, map=None))
    ...
    def SaveState(self, config_parser):
        ...
        for i, dfn in enumerate(self.columns):
            ...
            config_parser.set(section, '%s_width' % dfn.attribute,
                              str(col.GetWidth()))
