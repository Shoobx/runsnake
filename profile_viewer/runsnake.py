...
from profile_viewer import pstatsloader, pstatsadapter, meliaeloader, meliaeadapter
from profile_viewer import listviews
from profile_viewer import homedirectory
...
class MainFrame(wx.Frame):
    ...
    def CreateMenuBar(self):
        ...
        menu.Append(ID_OPEN_MEMORY,
                    _('Open &Memory'), _('Open a Meliae memory-dump file'))
        ...
        self.rootViewItem = menu.Append(ID_ROOT_VIEW,
                                        _('&Root View (Home)'),
                                        _('View the root of the tree'))
        self.backViewItem = menu.Append(ID_BACK_VIEW,
                                        _('&Back'),
        ...
        self.deeperViewItem = menu.Append(ID_DEEPER_VIEW,
                                          _('&Deeper'),
                                          _('View deeper squaremap views'))
        self.shallowerViewItem = menu.Append(
            ID_SHALLOWER_VIEW,
            _('&Shallower'), _('View shallower squaremap views'))
    ...
    def LoadRSRIcon(self):
        try:
            from profile_viewer.resources import rsricon_png
...
#        self.Bind(wx.EVT_TOOL, self.OnOpenFile, id=ID_OPEN)
    ...
    def load(self, *filenames):
        ...
        except (IOError, OSError, ValueError, MemoryError):
            self.SetStatusText(
                _('Failure during load of %(filenames)s: %(err)s') % dict(
                    filenames=" ".join([repr(x) for x in filenames]), err=err))
    ...
    def load_coldshot(self, dirname):
        from profile_viewer import coldshotadapter
    ...
    def LoadState(self, config_parser):
        ...
        if not config_parser:
            return
        if (not config_parser.has_section('window')
                or (config_parser.has_option('window', 'maximized')
        ...
        try:
            ...
            log.error("Unable to load window preferences, ignoring: %s",
